...
const SignsView: React.FC<{ speak:(t:string)=>void; onBack:()=>void }> = ({ speak, onBack }) => {
  ...
  useEffect(() => {
    speak(`Vamos aprender sobre o sinal de ${lesson.name}! ${lesson.description} Por exemplo: ${lesson.example}. ${lesson.tip}`);
    ...
    <div className="flex flex-col gap-5">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <button onClick={() => speak(`${lesson.description} ${lesson.tip}`)}
      ...
      <button onClick={() => { voiceService.cancel(); onBack(); }}
...
export const ExerciseView: React.FC<Props> = ({ addStars, speak, onBack }) => {
  ...
  // Narrate the question — fires once per question, after first user gesture
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      if (cur.type === 'word') {
        const withGap = cur.word!.split('').map((l,i) => i===cur.missingIndex ? '...' : l).join(' ');
        speak(`Que letra falta? ${withGap}`);
      } else if (cur.type === 'math') {
        const spoken = (cur.question ?? '')
          .replace(/\+/g,' mais ').replace(/-/g,' menos ')
          .replace(/x/g,' vezes ').replace(/÷/g,' a dividir por ')
          .replace(/_/g,' quanto? ');
        speak(`Matemática! ${spoken}`);
      } else {
        speak(`Qual é o próximo número? ${(cur.question ?? '').replace(/-/g,'...').replace('_','ponto de interrogação')}`);
      }
    }, 300);
  ...
  const handleAnswer = (opt: string) => {
    ...
    if (opt === cur.answer) {
      ...
      addStars(15);
      const msgs = [
        `Muito bem! Fantástico!${cur.type==='word' ? ` A palavra é ${cur.word}!`:''}`,
        `Uau, acertaste! Boa!`,
        `Espetacular! Muito bem!`,
        `Sim! Isso mesmo! Parabéns!`,
      ];
      speak(msgs[Math.floor(Math.random()*msgs.length)]);
    } else {
      const msgs = [
        `Quase! A resposta certa era ${cur.answer}. Não desistas!`,
        `Boa tentativa! Era ${cur.answer}. Continua!`,
        `Não faz mal! A resposta é ${cur.answer}. Vamos à próxima!`,
      ];
      speak(msgs[Math.floor(Math.random()*msgs.length)]);
    }
  };
  ...
  const nextQ = () => {
    ...
    if (qIndex < total-1) {
      ...
      setFinished(true);
      speak(`Fantástico! Completaste todos os exercícios! Acertaste ${correct} de ${total}!`);
    }
  };
  ...
  if (finished) return (
    <div className="rounded-[28px] border-1.5 border-white/10 bg-white/10 p-10 text-center backdrop-blur-xl">
      ...
      <div className="flex flex-wrap justify-center gap-4">
        <button onClick={() => { voiceService.cancel(); window.location.reload(); }}
  ...
  return (
    <div className="rounded-[28px] border-1.5 border-white/10 bg-white/10 p-6 sm:p-8 text-center backdrop-blur-xl">
      ...
      {/* Emoji */}
      <motion.div key={qIndex+'-e'} initial={{scale:0.5,opacity:0}} animate={{scale:1,opacity:1}} className="mb-5 text-7xl">
        {cur.emoji}
      </motion.div>
                ...
                {part === '_' ? (
                  answered ? (
                    <motion.span initial={{scale:0,y:-20}} animate={{scale:1,y:0}}
                      transition={{type:'spring',stiffness:300}}
                      className={selected===cur.answer?'text-emerald-400':'text-red-400'}
                    >{cur.answer}</motion.span>
                  ) : (
                    <motion.span animate={{opacity:[1,0.3,1]}} transition={{duration:1.2,repeat:Infinity}}
                      className="text-yellow-400">?</motion.span>
                  )
                ) : <span>{part}</span>}
              </div>
